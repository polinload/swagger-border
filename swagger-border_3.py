...
def generate_param_combinations(parameters, url, csv_params=None):
    ...
    csv_params = csv_params or {}
    candidates_list = []
    param_meta = []  # keep metadata for each param (name, in, type)

    for param in parameters:
        param_name = param.get('name')
        param_in = param.get('in')
        param_type = param.get('type') or (param.get('schema') or {}).get('type')
        param_meta.append((param_name, param_in, param_type))
    ...
    for combo in itertools.product(*candidates_list):
        filled = {}
        path_params = {}
        for (param_name, param_in, param_type), val in zip(param_meta, combo):
